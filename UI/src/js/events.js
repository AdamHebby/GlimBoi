...
function loadEvents() {
  ...
  $('#modalRaffleList').on('show.bs.modal	', function(e) {
    ...
    const set = EventHandle.getRaffleUsers();
    set.forEach((element) => {
  ...
  if (arrayOfEvents.includes('poll')) {
    ...
    if (GUI) {
      ...
      EventHandle.startPoll({question: message, options: pollOptions, user: user}, 25000).then((data) => {
        if (data == 'NOPOLL') {
          const pollError = document.getElementById('PollModalText').innerHTML = 'No Question was selected.';
          setTimeout(() => {
            pollError.innerText = '';
          }, 3000);
        }
      ...
      const questionEnd = stringMessage.indexOf('?');
      console.log(questionEnd);
      const question = stringMessage.substring(questionEnd + 1);
      const possibleAnswers = question.split('|');
