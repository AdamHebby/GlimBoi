...
function checkNewCommand() {
  ...
  const commandPoints = Number($('#addCommandPoints').text());
  const commandUses = Number($('#addCommandUses').text());
  let repeat = document.getElementById('commandRepeatableChoice').value;
  const commandRank = document.getElementById('rankChoiceAdd').value;
  ...
  function errorMessageCommandModal(message, errLocation) {
    const addCommandName = document.getElementById(errLocation).parentElement;
    const cmdErrorMessage = document.getElementById('errorMessageAdd');
...
function checkRemoveCommand() {
  let commandToBeRemoved = $('#commandRemoveInput').val().toLowerCase();
  ...
  try {
    CommandHandle.findCommand(commandToBeRemoved).then((data) => {
      if (data !== null) {
        ...
        const removeCommandMessageError = document.getElementById('removeCommandMessage');
...
function checkEditCommand() {
  ...
  const editErrorMessage = document.getElementById('editCommandMessage');
...
function editCommand() {
  ...
  const commandPoints = Number($('#editCommandPoints').text());
  const commandUses = Number($('#editCommandUses').text());
  const commandRank = document.getElementById('rankChoiceEdit').value;
  ...
  CommandHandle.findCommand(commandToBeEdited).then((data) => {
    if (data !== null) {
      const filteredData = table // A set of functions that removes the command, adds it back, and redraws the table.
          .rows()
          .indexes()
          .filter(function(value, index) {
  ...
  // Shows an error in the modal
  function errorMessageCommandModal(message, errLocation) {
    const editCommandName = document.getElementById(errLocation).parentElement;
    const cmdErrorMessageEdit = document.getElementById('errorMessageEdit');
