...
function getSettings() {
  try { // Check if the file exists.
    const raw = fs.readFileSync(appData[1] + '/data/settings.json');
...
function showSettings() {
  // Points - - -
  const slider = document.getElementById('initialValueSlider');
  slider.value = settings.Points.StartingAmount;
  const output = document.getElementById('initialValueOutput');
  output.innerHTML = slider.value;
  slider.oninput = function() {
    output.innerHTML = this.value;
  };
  document.getElementById('pointsNewName').value = settings.Points.name;
  const rateSlider = document.getElementById('rateValueSlider');
  rateSlider.value = settings.Points.accumalation;
  const rateOutput = document.getElementById('rateValueOutput');
  rateOutput.innerHTML = rateSlider.value;
  rateSlider.oninput = function() {
    rateOutput.innerHTML = this.value;
  };
  // Chat - - -
  const loggingswitch = document.getElementById('loggingEnabled');
  ...
  // repeat handlers
  const repeatDelay = document.getElementById('repeatDelaySlider');
  repeatDelay.value = settings.Commands.repeatDelay;
  const repeatDelayValue = document.getElementById('repeatDelayValue');
...
function saveSettings() {
  function getCooldown() {
    const value = document.getElementById('sel1').value;
    switch (value) {
      case 'None (default)':
        return 0;
        break;
  ...
  function getRepeatProtection() {
    const value = document.getElementById('repeatProtect').value;
    switch (value) {
