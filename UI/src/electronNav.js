    ...
    linkEl.addEventListener('click', (e) => {
      ...
      if (href) {
        ...
        fs.readFile(`${appData[0]}/${href}`, (err, data) => {
          ...
          if (linkEl.id == 'CommandLink') {
            loadCommandTable();
          } // Builds the data table
          if (linkEl.id == 'GlimBoiHeader') {
            getBasicData(); rememberID();
          } // Builds the homepage charts and check for auth ino for the buttons
          if (linkEl.id == 'EventsLink') {
            loadEvents();
          }
          if (linkEl.id == 'UsersLink') {
            loadUsers();
          }
          if (linkEl.id == 'PointsLink') {
            getPoints();
          }
          if (linkEl.id == 'SettingsLink') {
            showSettings();
          }
          if (linkEl.id == 'ChatLink') {
            loadChatWindow();
          } // Builds the data table
