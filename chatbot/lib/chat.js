...
var heartbeat; var stats; // heartbeat and stats intervals
...
var repeatSpamProtection = 15; var repeatDelay = 600000; // The users repeat settings. Default is at least 5 messages between repeats, attempt every 5 min
 ...
 * @return Returns array of channel objects
 */
...
function join(access_token, channelID) {
  try {
    connectToGlimesh(access_token, channelID);
  } catch (e) {
...
function connectToGlimesh(access_token, channelID) {
  ...
  connection.on('open', function open() { // When the connection opens...
    ...
    checkForUsers = setInterval(() => {
      ...
      const currentUsersFiltered = [...new Set(currentUsers)];
      currentUsers = [];
      ...
      if (chatMessage[4].status !== undefined) {
        ...
        try {
          if (chatMessage[4].result.data !== undefined) {
            const userChat = chatMessage[4].result.data.chatMessage.user.username;
            const messageChat = chatMessage[4].result.data.chatMessage.message;
            const userID = Number(chatMessage[4].result.data.chatMessage.user.id);
            ...
            if (messageChat.startsWith('!')) { // If it is a command of some sort...
              ...
              const message = messageChat.split(' ');
              switch (message[0]) {
                  ...
                  switch (message[1]) {
                    ...
                    case undefined:
                      UserHandle.findByUserName(userChat.toLowerCase()).then((data) => {
                        if (data == 'ADDUSER') {
                          filterMessage('That user does not exist in the database yet. Type !user new ' + userChat.toLowerCase(), 'Glimboi');
                        } else {
...
function sendMessage(data) {
  const msgArray = ['6', '7', '__absinthe__:control', 'doc']; // array of data to send to glimesh
  ...
  const test = JSON.stringify(msgArray); // make it sendable (json)
  try {
    console.log(test);
...
function glimboiMessage(data) {
  const msgArray = ['6', '7', '__absinthe__:control', 'doc'];
  msgArray.splice(4, 0, {'query': 'mutation {createChatMessage(channelId:"' + chatID +'", message:{message:"'+data+'"}) {message }}', 'variables': {}});
  const test = JSON.stringify(msgArray);
  try {
    console.log(test);
 ...
 */
function test() {
  const test = JSON.stringify(['1', '1', '__absinthe__:control', 'doc', {'query': 'subscription{ chatMessage { user { username avatar } message } }'}]);
  connection.send(test);
  console.log(test);
}
...
function logMessage(user, message, avatar) {
  const adminClass = (user === botname) ? 'admin_chat' : '';
  ...
  const scroll = document.getElementById('chatContainer');
...
function delUserChat(user) {
  const exists = UserHandle.findByUserName(user);
  exists.then((data) => {
    if (data == 'ADDUSER') {
...
function addQuoteChat(data, user) {
  ...
  const trimMessage = 10 + user.length + 2;
