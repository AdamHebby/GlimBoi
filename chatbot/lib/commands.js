...
var startCD = new Date(); var timeCD; // When a command is activated to the next command. Subtract now from then.
...
function addCommand(commandName, arguements, commandData, uses, points, rank, special, repeat) {
  const newCommand = new Command(commandName, arguements, commandData, Number(uses), Number(points), rank, special, repeat);
  try {
  ...
  commandsDB.update({commandName: commandName}, {$set: {arguements: arguements, message: commandData, uses: Number(commandUses), points: Number(commandPoints), rank: commandRank, special: special, repeat: repeat}}, {}, function(err, numReplaced) {
    ...
    for (let index = 0; index < commands.length; index++) {
      if (commandName == commands[index].commandName) {
        ...
        const repeatExists = findRepeat(commandName);
...
function checkCommand(data) {
  const message = data.message.split(' '); // splits by space
  message[0] = message[0].substring(1); // The command without the !
  console.log(message);
  timeCD = new Date();
  const CD = timeCD - startCD; // check the time since the last command was activated. (cooldown check)
  console.log(CD);
...
function randomRepeatCommand() {
  const index = Math.floor(Math.random()*repeatableArray.length);
 ...
 */
function getTime() {
  const theTime = new Date().toTimeString();
  return theTime;
}
