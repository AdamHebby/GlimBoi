...
async function startRaffle(time) {
  return new Promise((resolve) => {
    ...
    raffleTimer.timer = setTimeout(() => {
      arrayOfEvents = arrayOfEvents.filter(function(e) {
        return e !== 'raffle';
      }); // removes from current events
      ...
      const index = Math.floor(Math.random()*raffleUsers.length);
      const winner = raffleUsers[index];
      raffleUsers = [];
...
function glimDropRealm(user, data) {
  ...
  const result = glimChance(); // get a random effect
  console.log(result);
