 ...
 * @return If the user doesn't exist on GLimesh returns code INVALIDUSER
 * @return If the exists returns USEREXISTS
 */
async function addUser(user, inModal) {
  const newUser = await new Promise((done) => {
      ...
      if (docs.length == 0) {
        ...
        ApiHandle.getUserID(user).then((ID) => {
          if (isNaN(ID) == true || ID == null || typeof ID == Object) {
            ...
            console.log(ID);
            const tempUser = new User(user.toLowerCase(), ID); // makes the user. L I F E !
 ...
 * @return If successful returns the user.
 * @return If the user does not exist returns ADDUSER
...
/**
 * @return All the users in the DB. Sent as an array
 */
 ...
 * @return {array} The user that was removed
 */
