    ...
    newquote.quoteID.then((data) => {
      ...
      if (data == 'ADDUSER') {
        ...
        if (data[0].quotes.length == 0) {
          ...
          const count = data[0].quotes.length - 1;
          console.log(count);
...
async function generateID(quoteName) {
  ...
  const usedID = await UserHandle.findByUserName(quoteName); // Gets the number of quotes of this user. Is the user is not existent return ADDUSER
  return usedID;
}
  ...
  return new Promise((resolve) => {
    quotesDB.find({}, function(err, docs) {
      if (docs.length == 0 || docs == undefined) {
        ...
        const randomQuote = Math.floor(Math.random() * docs.length);
