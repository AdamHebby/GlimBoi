  ...
  if (serverisOn == true) { // If the server is already running we just send them to glimesh again.
    ...
    var server = http.createServer((req, res) => { // new server!
      ...
      if (req.url.startsWith('/auth')) { // if they somehow get to this url...
        ...
        const tokenURL = new URL('https://glimesh.tv/api/oauth/token');
        const params = {
 ...
 */
function Auth() {
  const authscheme = readAuth().then((data) => {
...
async function refreshToken(refresh_token, client_id, client_secret) {
  return new Promise((resolve) => {
    const url = new URL('https://glimesh.tv/api/oauth/token'); // Fetch wants a special url
    const params = {grant_type: 'refresh_token', refresh_token: refresh_token, redirect_uri: 'http://localhost:3000/success', client_id: client_id, client_secret: client_secret};
          ...
          res.json().then((data) => {
            try {
              ...
              authDB.update({}, {$set: {access_token: data.access_token, refresh_token: data.refresh_token, created_at: data.created_at, expire: data.expires_in}}, {multi: true}, function(err, numReplaced) {
                ...
                refreshed = true;
                resolve('SUCCESS');
              }); // Lets the orignal function know that everything worked
            } catch (e) {
              console.log(e);
